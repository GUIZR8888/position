...
# 1、构建数据库
db = SQLDatabase.from_uri("mysql+pymysql://root:1234@localhost:3306/job")
# 2、构建大模型
model = ChatOpenAI(
    temperature=0.95,
    model="glm-4",
    openai_api_key="fcd02a02ad1343cb77d547e313a14cdc.KsXK5ughrSvCuKco",
...
prompt = PromptTemplate.from_template("现在你是一个专业的数据库问答专家，你只能对数据库的这些问题做回答，用户的提问是{input},数据库对问题的回答是{answer},你需要对数据库的回答加以美化")
